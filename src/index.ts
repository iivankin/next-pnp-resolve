/**
 * Copied from https://github.com/vercel/nft/blob/main/src/resolve-dependency.ts
 * With small patch from here https://github.com/vercel/nft/pull/152/commits/b484e2da1fc5a896bf4c018bea357ddcc013b5f8#diff-2eebdbf3ec07cd4b5bb39605dd391ebc4cc1dee03f006179e126de1df3404e6fR179
 * It will be 99% smaller if they exported all the functions.
 */

import { isAbsolute, resolve, sep } from 'path';
import { type Job } from '@vercel/nft/out/node-file-trace';

// node resolver
// custom implementation to emit only needed package.json files for resolver
// (package.json files are emitted as they are hit)
export async function resolveDependency(specifier: string, parent: string, job: Job, cjsResolve = true): Promise<string | string[]> {
...
async function resolveFile(path: string, parent: string, job: Job): Promise<string | undefined> {
	...
	if (
		...
		path.substr(job.base.length).indexOf(sep + 'node_modules' + sep) === -1 &&
...
async function resolveDir(path: string, parent: string, job: Job) {
	...
	if (pkgCfg && typeof pkgCfg.main === 'string') {
		const resolved =
			(await resolveFile(resolve(path, pkgCfg.main), parent, job)) || (await resolveFile(resolve(path, pkgCfg.main, 'index'), parent, job));
		if (resolved) {
		...
		if (pjsonBoundary) {
			...
			if (pkgCfg && pkgImports !== null && pkgImports !== undefined) {
				...
				if (importsResolved) {
					...
					else if (!(await job.isFile(importsResolved))) throw new NotFoundError(importsResolved, parent);
					if (importsResolved) {
...
async function resolvePackage(name: string, parent: string, job: Job, cjsResolve: boolean): Promise<string | string[]> {
	...
	if (job.conditions) {
		...
		if (pjsonBoundary) {
			...
			if (pkgCfg && pkgCfg.name && pkgCfg.name === pkgName && pkgExports !== null && pkgExports !== undefined) {
				...
				if (selfResolved) {
					if (cjsResolve) selfResolved = (await resolveFile(selfResolved, parent, job)) || (await resolveDir(selfResolved, parent, job));
					else if (!(await job.isFile(selfResolved))) throw new NotFoundError(selfResolved, parent);
				}
				if (selfResolved) await job.emitFile(pjsonBoundary + sep + 'package.json', 'resolve', parent);
			}
		}
	}

	// Here is our magic patch
	...
	while ((separatorIndex = packageParent.lastIndexOf(sep)) > rootSeparatorIndex) {
		...
		if (job.conditions && pkgExports !== undefined && pkgExports !== null && !selfResolved) {
			...
			if (!job.exportsOnly)
				legacyResolved =
					(await resolveFile(nodeModulesDir + sep + name, parent, job)) || (await resolveDir(nodeModulesDir + sep + name, parent, job));
			...
			if (resolved) {
				if (cjsResolve) resolved = (await resolveFile(resolved, parent, job)) || (await resolveDir(resolved, parent, job));
				else if (!(await job.isFile(resolved))) throw new NotFoundError(resolved, parent);
			}
			if (resolved) {
				await job.emitFile(nodeModulesDir + sep + pkgName + sep + 'package.json', 'resolve', parent);
				if (legacyResolved && legacyResolved !== resolved) return [resolved, legacyResolved];
				return resolved;
			}
			...
			const resolved =
				(await resolveFile(nodeModulesDir + sep + name, parent, job)) || (await resolveDir(nodeModulesDir + sep + name, parent, job));
			if (resolved) {
				if (selfResolved && selfResolved !== resolved) return [resolved, selfResolved];
				return resolved;
			}
		}
	}
